...
class ApiPostController extends Controller
{
    ...
    public function searchByFiled(Request $request)
    {
        $field = $request->field;
        $name = $request->name;
        if ($field === "category_id") {
            ...
            if ($post) {
                return response()->json(['message' => 'Đã tìm thấy bài đăng', 'data' => PostResource::collection($post)], 200);
            } else {
                return response()->json(['message' => 'Không tìm thấy bài đăng phù hợp'], 404);
            }
        } else {
    ...
    public function relatedPost($postId)
    {
        ...
        if (!$currentPost || $currentPost->status === "Bản nháp") {
            return response()->json(['message' => 'Không tìm thấy bài đăng có liên quan'], 404);
        }
